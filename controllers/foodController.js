...
exports.CreateFood = async (req, res) => {
    try {
        if (!req.body.name || !req.body.restaurant_name || !req.body.price || !req.body.rating || !req.body.image || !req.body.location || !req.body.expected_delivery) {
            return res.status(400).send({ error: "All fields are required" });
        }
        const { name, restaurant_name, price, rating, image, location, expected_delivery } = req.body;
        const food = new Food({ name, restaurant_name, price, rating, image, location, expected_delivery });
        await food.save();
...
exports.UpdateFood = async (req, res) => {
    try {
        ...
        const food = await Food.updateOne(
            { _id: new ObjectId(id) },
            { $set: { name, restaurant_name, price, rating, image, location, expected_delivery } }
        );
